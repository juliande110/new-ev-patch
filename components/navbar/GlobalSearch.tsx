...
const CollectionItem: FC<Props> = ({ collection, handleSelectResult }) => {
  ...
  return (
    <Link
      href={`/${collection.chainRoutePrefix}/collection/${collection.collectionId}`}
...
>(({ children, ...props }, forwardedRef) => {
  ...
  useEffect(() => {
    ...
    const getSearchResults = async () => {
      setSearching(true)
      let res = await fetch(
        `/api/globalSearch?query=${debouncedSearch}&collectionsSetId=${collectionId}&searchChain=${searchChain}`
      ).then((res) => res.json())
  ...
  }, [debouncedSearch, searchChain, collectionId])
