  ...
  return (
    ...
    >
          ...
          content={
            <Flex direction="column" css={{ gap: '$2' }}>
                  ...
                  {token?.ownership?.floorAsk?.price?.amount?.decimal ? (
                    <>
                      <FormatCryptoCurrency
                        ...
                        logoHeight={14}
                      />
                      <Text style="subtitle3">{token?.ownership?.floorAsk?.price?.currency?.symbol}</Text>
                    </>
                  ) : '-'}
                </Flex>
              </Flex>
              <Flex justify="between" css={{ gap: '$2' }}>
                ...
                >
                  {token?.ownership?.floorAsk?.price?.netAmount?.decimal ? (
                    <>
                      <FormatCryptoCurrency
                        amount={
                          token?.ownership?.floorAsk?.price?.netAmount?.decimal
              ...
              {token?.ownership?.floorAsk?.price?.amount?.decimal ? (
                <>
                  <FormatCryptoCurrency
                    ...
                    logoHeight={14}
                  />
                  <Text style="subtitle2">{token?.ownership?.floorAsk?.price?.currency?.symbol}</Text>
                </>
              ) : '-'}
            </Flex>
          </Flex>
      ...
      <TableCell>
        ...
        >
          {token?.token?.collection?.floorAskPrice?.amount?.decimal ? (
            <>
              <FormatCryptoCurrency
                ...
                logoHeight={14}
              />
              <Text style="subtitle1">{token?.token?.collection?.floorAskPrice?.currency?.symbol}</Text>
            </>
          ) : '-'}
        </Flex>
      </TableCell>
              ...
              <AcceptBid
                ...
                buttonCss={{
                  ...
                  '&:hover': {
                    color: 'white',
                    background:
                      'linear-gradient(0deg, #2F2F32 0%, #161617 100%)',
                  },
                }}
                buttonChildren={
                  <Flex align="center" css={{ gap: '$2' }}>
