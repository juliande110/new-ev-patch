...
export default async function handler(req: Request) {
  ...
  const collectionsSetId = searchParams.get('collectionsSetId')
  ...
  if (searchChain) {
    ...
    if (chain) {
      searchResults = await searchSingleChain(chain, query, collectionsSetId)
    }
  }

  if (!searchResults.length) {
    fallbackResults = await searchAllChains(query, collectionsSetId)
  }
...
async function searchSingleChain(
  ...
  let queryParams: paths['/search/collections/v1']['get']['parameters']['query'] =
    {
      ...
      collectionsSetId: collectionsSetId as string,
      limit: 6,
    }
  ...
  const promise = fetcher(
    `${reservoirBaseUrl}/search/collections/v2`,
    queryData,
    headers,
  )
  ...
  if (isAddress) {
    const { data } = await fetcher(
      `${reservoirBaseUrl}/collections/v7?contract=${query}&collectionsSetId=${collectionsSetId}&limit=6`,
      {},
      headers,
    )
...
async function searchAllChains(query: string, collectionsSetId: string | null) {
  let searchResults: any[] = []
