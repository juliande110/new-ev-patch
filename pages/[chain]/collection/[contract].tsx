...
const CollectionPage: NextPage<Props> = ({ id, ssr }) => {
  ...
  const [collectionVerified, setCollectionVerified] = useState<boolean>(false)
  ...
  const [isLoadedTotalSupply, setIsLoadedTotalSupply] = useState<boolean>(false)
  ...
  useEffect(() => {
    if (id && !collectionVerified) {
      const verifyCollection = async () => {
        try {
          const response = await axios.get('/api/verifyCollection', {
            params: {
  ...
  }, [collectionVerified, id])

  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('/api/getTotalSupply', {
          params: {
            collectionId: id,
          },
        })
        if (response) {
          setIsLoadedTotalSupply(true)
          setTotalVaults(response?.data)
        }
      } catch (error) {
    ...
    return (
      <Layout>
        ...
        <Flex>
          ...
          >
            <StyledImage
              src={ssr?.collection?.collections?.[0]?.banner}
          ...
          >
            ...
            >
              <StatBox
                ...
                value={
                  `${Number(
                    collection?.volume?.['allTime']?.toFixed(2)
              ...
              {collection?.contractKind === 'erc1155' ? (
                <StatBox
                  title="Unique Assets"
